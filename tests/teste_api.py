...
def test_create_and_get_consumidor():
    """
    Testa a criação de um novo consumidor via POST e a sua listagem via GET
    """
    ...
    try:
        ...
        assert (
            "id" in created_consumidor
            ), "A resposta de criação não contém um 'id'."
        consumidor_id = created_consumidor["id"]
        assert created_consumidor["nome"] == new_consumidor["nome"], (
            f"Nome do consumidor criado ({created_consumidor['nome']})"
            f"diferente do enviado ({new_consumidor['nome']})."
        )
    ...
    try:
        ...
        assert response_get.status_code == 200, (
            f"Erro ao listar consumidores: {response_get.status_code}"
            f"- {response_get.text}"
        )
        ...
        for consumidor in all_consumidores:
            if consumidor["id"] == (
                ...
                break
        assert found, (
            f"Consumidor criado com ID {consumidor_id} e nome"
            f"'{new_consumidor['nome']}' não encontrado na lista."
        )
        print(
            f"Consumidor com ID {consumidor_id} e"
            f"nome '{new_consumidor['nome']}' encontrado na lista."
            )
    except requests.exceptions.ConnectionError as e:
